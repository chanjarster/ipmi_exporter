...
import (
	...
	"encoding/json"
	"fmt"
	fsutil "github.com/prometheus-community/ipmi_exporter/sidecar/utils/fs"
	"strings"
	"sync"
	"time"
...
func (s *sidecarService) assertZoneIdMatch(zoneId string) error {
	...
	if s.boundZoneId != zoneId {
		return errs.ValidateErrorf("Current ipmi-exporter bound zoneId=%s, command zoneId=%s, mismatch",
